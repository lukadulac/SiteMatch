...
export async function withdrawApplicationAction(applicationId: string) {
	...
	if (result.error) {
		redirect(
			`/dashboard/provider/applications?applicationError=${encodeURIComponent(
				result.error,
			)}`,
		);
	}
	...
	redirect(
		`/dashboard/provider/applications?applicationStatus=${encodeURIComponent(
			result.data?.status === "withdrawn"
				? "Proposal withdrawn."
				: "Proposal already withdrawn.",
		)}`,
	);
}
