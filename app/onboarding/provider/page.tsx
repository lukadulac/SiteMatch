...
export default async function ProviderOnboardingPage() {
  ...
  const { data: providerProfile } = await supabase
    ...
    .select(
      "provider_type, headline, bio, years_of_experience, portfolio_url, availability",
    )
    .eq("user_id", user.id)
    .maybeSingle();

  if (isProviderProfileComplete(providerProfile)) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-5xl flex-col justify-center px-5 py-6 sm:px-8 lg:px-10">
      <section className="grid overflow-hidden rounded-[2rem] border border-line bg-panel shadow-[0_36px_90px_rgba(0,0,0,0.34)] lg:grid-cols-[0.86fr_1.14fr]">
        <div className="flex flex-col justify-between bg-[linear-gradient(180deg,#101629_0%,#1a2240_50%,#202c51_100%)] p-8 sm:p-10">
          <div className="space-y-5">
            ...
            <p className="max-w-md text-sm leading-7 text-[#d6def6] sm:text-base">
              Add the key details that help clients understand your experience,
              your offer, and your availability.
            </p>
          </div>
          <div className="rounded-[1.5rem] border border-white/12 bg-black/18 p-5 backdrop-blur-sm">
            ...
            <p className="mt-2 text-sm leading-6 text-[#eef2ff]">
              Once this is complete, you will go straight to your provider
        ...
        <div className="p-8 sm:p-10 lg:p-12">
          <ProviderOnboardingForm providerProfile={providerProfile} />
        </div>
      </section>
    </main>
  );
